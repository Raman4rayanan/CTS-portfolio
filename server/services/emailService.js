...
const sendViaBrevo = async (toEmail, subject, htmlContent) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error(`Brevo API Error sending to ${toEmail}:`, JSON.stringify(errorData));
      return false;
    }
  } catch (error) {
    console.error(`Network error sending to ${toEmail}:`, error);
    return false;
  }
};
  ...
  try {
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
        ...
        <p style="font-size: 16px; color: #333;">Please use the following 4-digit verification code to complete your sign-up process:</p>
